...
The chunk schema has moved from embedded `slug`/`type` fields to edge-based `has_chunk` relationships:

**Before:** Chunks stored `slug` and `type` directly as fields.
**After:** Chunks have `page_posts:xxx --has_chunk--> chunks:xxx_chunk_0` edges.
...
This recreates all chunks and creates `has_chunk` edges.
...
```sql
SELECT count() AS cnt FROM has_chunk GROUP ALL;
SELECT * FROM has_chunk LIMIT 10;
```
...
2. Documents are re-indexed, creating chunks with proper `has_chunk` edges

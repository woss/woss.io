...
function buildQuestionEventDedupeKey(properties: unknown): string | null {
  ...
  const toolInfo =
    record.tool && typeof record.tool === 'object' ? (record.tool as Record<string, unknown>) : undefined;
...
async function sendNotification(options: NotificationOptions, runtime: NotificationRuntime): Promise<void> {
...
export const NotifyPlugin: Plugin = async (ctx) => {
  ...
  const notifyQuestionIfNeeded = async (dedupeKey: string | null): Promise<void> => {
    if (
      dedupeKey &&
      !shouldSendDedupedNotification(recentQuestionNotifications, dedupeKey, QUESTION_DEDUPE_WINDOW_MS)
    ) {
      return;
    }
  ...
  const notifySessionReadyIfNeeded = async (sessionID: unknown): Promise<void> => {
    ...
    await handleSessionIdle(client as OpencodeClient, normalizedSessionID, config, terminalInfo, notificationRuntime);
  };
  ...
  const notifyPermissionIfNeeded = async (properties: unknown): Promise<void> => {
    ...
    if (
      dedupeKey &&
      !shouldSendDedupedNotification(recentPermissionNotifications, dedupeKey, PERMISSION_DEDUPE_WINDOW_MS)
    ) {
      return;
    }
    ...
    event: async ({ event }: { event: Event }): Promise<void> => {
      ...
      switch (runtimeEvent.type) {
        ...
        case 'session.error': {
          ...
          const errorMessage = typeof error === 'string' ? error : error ? JSON.stringify(error) : undefined;
          if (sessionID) {

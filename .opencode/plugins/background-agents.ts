...
export interface DelegationManager {
  delegate(input: DelegateInput): Promise<DelegationRecord>;
  handleSessionIdle(sessionID: string): Promise<void>;
  readOutput(sessionID: string, id: string): Promise<string>;
  listDelegations(sessionID: string): Promise<DelegationListItem[]>;
  deleteDelegation(sessionID: string, id: string): Promise<boolean>;
  ...
  getUnreadCompletedDelegations(rootSessionID: string, limit?: number): DelegationRecord[];
  getRecentCompletedDelegations(sessionID: string, limit?: number): Promise<DelegationListItem[]>;
  getRootSessionID(sessionID: string): Promise<string>;
  debugLog(msg: string): Promise<void>;
}

class DelegationManagerImpl {
...
/** Create DelegationManager — factory wrapper to avoid Bun/JS Core [[Construct]] bug */
export function createDelegationManager(
  ...
  return new DelegationManagerImpl(client, baseDir, log, options);
}
...
export const BackgroundAgentsPlugin: Plugin = async (ctx) => {
  ...
  const manager = createDelegationManager(client as OpencodeClient, baseDir, log);

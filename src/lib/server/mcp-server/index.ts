...
mcpServer.registerTool(
  'get_posts',
  {
    ...
    inputSchema: {
      from: z.string().optional().describe('ISO date lower bound (inclusive), e.g. "2026-01-01"'),
      to: z.string().optional().describe('ISO date upper bound (inclusive), e.g. "2026-06-30"'),
  ...
  async ({ from, to, sort, order, last }) => {
    ...
    // Date range filter (applied post-query since DB filtering by date range isn't supported yet)

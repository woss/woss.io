 ...
 *
 * Env is read via `readEnv()` — `$env/dynamic/private` first, then
 * `process.env`. In dev, `.env` is only surfaced through
 * `$env/dynamic/private` (SvelteKit's Vite `loadEnv`), so raw `process.env`
 * misses it. In built adapter-node mode, `$env/dynamic/private` resolves to
 * `process.env`, so the Docker-baked `DD_VERSION` (the image tag, passed via
 * the Dockerfile build-arg) and any runtime env are picked up. Nothing is
 * baked into the bundle at build time, so docker builds succeed without a
 * `.env` file. See `readEnv()` below.
 */
 ...
 *
 * Read via `readEnv()` inside startSdk(), which runs at module scope before
 ...
 *
 * `DD_SERVICE`, `DD_ENV`, and `DD_VERSION` are read at runtime via
 * `readEnv()` (`$env/dynamic/private` → `process.env`). In built adapter-node
 * mode this resolves to `process.env` — the Docker image bakes `DD_VERSION`
 * (the image tag) via `ENV`, so telemetry carries the exact deploy version.
 * `service.name` falls back to `woss-io` (SERVICE_NAME_FALLBACK) when
 * `DD_SERVICE` is unset; `deployment.environment` and `service.version` are
 * only included when their variables are defined.
 */
function createResource(): ReturnType<typeof resources.resourceFromAttributes> {
  const attributes: Record<string, string> = {
    'service.name': readEnv('DD_SERVICE') || SERVICE_NAME_FALLBACK,
  };
  const ddEnv = readEnv('DD_ENV');
  if (ddEnv) {
    attributes['deployment.environment'] = ddEnv;
  }
  const ddVersion = readEnv('DD_VERSION');
  if (ddVersion) {
